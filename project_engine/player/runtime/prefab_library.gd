...
func load_prefab(abs_path: String) -> PackedScene:
	...
	# Use ResourceLoader.exists rather than FileAccess so this works after
	# export: Godot rewrites .tscn to .scn in the resource pack and leaves a
	# .remap sidecar, and only ResourceLoader knows to follow the remap.
