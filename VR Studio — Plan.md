...
Branch: `feature/vr-studio` (was `claude/sweet-bardeen-mjgzvk`). It has `ci/build-and-release` and `feature/beat-detection` merged in.
...
- **v1 files still load:** their `cubic` is rewritten to `ease` on load (`ScriptFormat.upgrade`), because v1's cubic was a smoothstep. The player accepts versions 1–2; exporters write 2.
...
- **Commands, contexts, bindings:** `player/input/input_bindings.gd` has the 18 player commands, the contexts (menus > free > locked > play, with Studio's two above free since M1; the highest active context that uses an input owns it, a stick counting as one input), and default bindings that reproduce the player's old hard-coded buttons exactly. Only the user's changes are saved (`user://input_bindings.json`); changes for commands a version doesn't know are kept.
...
### Studio skeleton (milestone M1)
- **Edit model** (`studio/model/edit_model.gd`, `EditModel`): the script JSON in memory, changed only through commands: `set_spawn_transform`, `set_config` (a field or a path into it; null removes), `set_key` (makes the track if needed, replaces a key at the same time keeping its interp and handles), `move_key`, `delete_key` (the last key takes its track), `add_object` (optional despawn), `remove_object` (with its children, their tracks and despawns). A command is a list of path changes with old and new values; undo / redo replay them, a command that changes nothing isn't recorded, and new values are stored as JSON reads them (numbers as floats) so they compare with the file's. `changed(structural)` fires after every do / undo / redo.
  - **Saving:** validated with `ScriptFormat.load_from_dict` (new; `load_from_string` uses it) before writing; written to `.tmp` and renamed. While the document is at its loaded state (untouched, or everything undone) the **original text** is written back, since `JSON.stringify` doesn't reproduce hand-written or exported files byte for byte; after edits, stringify with the file's own indent and final newline, keys in order. v1 scripts are upgraded on load (`ScriptFormat.upgrade`, was `_upgrade`) and saved as v2 once edited. Dirty = undo depth differs from the saved one (a saved state reachable only by redo is gone once something new is done).
- **Runner patching** (`ScriptRunner.apply_edit(data, structural)`): the app hands it a fresh `EditModel.timeline()` after each change. Non-structural (keys): the timeline is swapped in place and tracks re-evaluated at the playhead, nothing respawned. Structural (spawn events, configs, adding / removing objects, `.reactive` tracks whose spin keys are read at spawn): `_reconcile_swap`, which respawns only objects whose spawn event changed (by value). `set_live_reload(false)`: Studio doesn't watch the file, or its own save coming back could reload over newer edits.
- **Seeking past cuts** (was a prerequisite): `ScriptRunner.cut_at(events, t)` / `cut_at_playhead()` give the cut in effect; after every seek the Stage (`_restore_cut`) puts the viewer there without a fade, or home before the first cut, but only when the seek lands under a *different* cut than the current one, so a viewer who walked off isn't pulled back by a ±10 s seek. Applies to the player too (seeking forward over a cut now applies it as well).
- **Studio scene** (`studio/studio.tscn` + `studio.gd`): the Stage instance, a CanvasLayer with the desktop status, and the wrist panel's scene swapped for the same status in compact form (`studio/ui/studio_status.gd`: mode chip, piece title, ● unsaved, time, the last action; keyboard hints on the desktop). CLI: `--piece <json | video with sidecar>`, `--start`, `--vr`, `--desktop`, or a plain file argument. Edit mode enables desktop flight (WASD, E/Q: the run's InputMap swaps Space/Ctrl for E/Q, since Space plays and Ctrl is for shortcuts).
- **Studio's commands** live in the shared `InputBindings` table with `"app": "studio"` (`command_app()`) and two new contexts above the player's: `studio` (always on in Studio) and `studio_edit` (on top in Edit). Studio turns the router's default `play` context off, so no player command fires there; the player never turns Studio's on, and its Controls tab lists only its own sections. Defaults: see the README table. No clashes (tested).
- **Launch:** (updated after M2, when the CI branch was merged) Studio has its own build: the *Studio Windows* / *Studio Linux* presets in the now committed `export_presets.cfg` carry the custom feature `studio`, and `project.godot` has `application/run/main_scene.studio="res://studio/studio.tscn"`, so that exe starts Studio: `ShaderPlayerVR-Studio.exe -- --piece x.json` (`build_and_run.bat studio [json]` builds and runs it). The player's presets exclude `studio/*`. CI exports both and ships Studio as portable zip / tar.gz. Checked here with `--export-pack` + `--main-pack` (no templates needed): the Studio pack opens Studio, the player pack the player without `studio/`. **Not tried with a full exported Windows build.**
- **Proof:** 176/176 tests (new: `test_edit_model.gd` 12, `test_studio_edits.gd` 2 for model → runner, a cut test in `test_script_runner.gd`, Studio bindings in `test_input.gd`); the byte-identical test fails if save always re-stringifies (tried). Round trip still passes. `checks/drive_studio.gd` (HEADLESS=1 prints; rendered it also shoots `studio_*.png`): copies forest_tunnel to `$WORK/studio_piece`, opens it, saves unchanged (**byte-identical**), Tab to Play (Ctrl+Z does nothing there) and back, keys the screen rig up and left (not respawned), Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y, a structural edit (glow off in the spawn config: `main_screen` respawns, the forest isn't touched), ←, Shift+←, Home, saves the edit, then opens the saved file in the **player**, which shows the rig where Studio put it. `drive_stage.gd` gained the seek-back lines (the rest prints as before); `drive.gd`, `drive_controls.gd` (now counts player commands only) and `drive_script_fx.gd` print the same as before M1. `run.sh` exports `REPO` and `WORK` for checks. Renders: `docs/studio/m1_studio.png`.
...
### Merged in after M2: CI and beat detection
- **`ci/build-and-release`:** the GitHub workflow (gde_gozen build, tests, exports, packages, releases on `v*` tags), `ci/` scripts, the committed `export_presets.cfg`, `gozen_loader.gd` (first autoload). Conflicts: README (both sections kept) and the forest_tunnel scene-states test (the CI branch's timing-free version kept). Studio got its own build target (see *Launch* under M1).
