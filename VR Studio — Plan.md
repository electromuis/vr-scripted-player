...
- **Launch:** `export_presets.cfg` is each user's own (gitignored), so there's no committed Studio preset: the same exe runs Studio with the scene as Godot's scene argument, `VRmviewer.exe res://studio/studio.tscn -- --piece x.json`; `build_and_run.bat studio [json]` does that. Checked here with the Linux editor binary (`--quit-after`): the piece opens. **Not tried with an exported Windows build.** Excluding `studio/` from the player's export is left for when Studio grows.
